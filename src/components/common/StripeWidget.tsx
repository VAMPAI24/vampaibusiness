...
const CheckoutForm = ({ clientSecret, onClose }: { clientSecret: string; onClose: () => void }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <PaymentElement />
      <button type="submit" disabled={!stripe || !elements || loading} className="bg-blue-500 text-white px-4 py-2 rounded w-full">
         ...
         <span>
          ...
          <h4 className={`text-[1.25em] font-[500] text-main-600 `}>
            ...
            {/* <strong className={`text-[1.5em] font-[500] `}>
              ...
              {formatterUSD.format(props.data.amount / 100)}{" "}
            </strong> */}
          </h4>
        </span>
    <Elements stripe={stripePromise} options={stripeOptions}>
      <CheckoutForm clientSecret={clientSecret} onClose={props.onClose} />
    </Elements>
    </div>
  ) : (
